...
void Sys_StartAsyncThread( void ) {
	...
	SetWaitableTimer( hTimer, &t, common->GetUserCmdMSec(), NULL, NULL, TRUE );
...
int WINAPI WinMain( HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow ) {
	...
	// The retail executable installs the CRT allocation callbacks before doing
	// any Win32 or common-system startup work.  Static constructors that run
	// before WinMain safely use Memory's CRT fallback (_LOAD_DLL build mode).
	Memory::InitAllocator( malloc, free, _msize );
#endif
...
void *idSysLocal::CreateWindowEx( const char *className, const char *windowName, int style, int x, int y, int w, int h, void *parent, void *menu, void *instance, void *param, int extStyle ) {
	return ::CreateWindowExA( extStyle, className, windowName, style, x, y, w, h, (HWND)parent, (HMENU)menu, (HINSTANCE)instance, param );
}

void *idSysLocal::GetDC( void *hWnd ) { return ::GetDC( (HWND)hWnd ); }
void idSysLocal::ReleaseDC( void *hWnd, void *hDC ) { ::ReleaseDC( (HWND)hWnd, (HDC)hDC ); }
void idSysLocal::ShowWindow( void *hWnd, int show ) { ::ShowWindow( (HWND)hWnd, show ); }
void idSysLocal::UpdateWindow( void *hWnd ) { ::UpdateWindow( (HWND)hWnd ); }
bool idSysLocal::IsWindowVisible( void *hWnd ) { return ::IsWindowVisible( (HWND)hWnd ) != FALSE; }
void idSysLocal::SetForegroundWindow( void *hWnd ) { ::SetForegroundWindow( (HWND)hWnd ); }
void idSysLocal::SetFocus( void *hWnd ) { ::SetFocus( (HWND)hWnd ); }
void idSysLocal::DestroyWindow( void *hWnd ) { ::DestroyWindow( (HWND)hWnd ); }
