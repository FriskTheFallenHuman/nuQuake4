...
protected:
	...
	virtual bool				Validate( const char *text, int textLength, idStr &report ) const;
...
static_assert( sizeof( rvDeclGuide ) == 0x2C4, "retail rvDeclGuide ABI drift" );
#endif
...
public:
	...
	virtual void				StartLoadingDecls( void );
	virtual void				FinishLoadingDecls( void );
	virtual void				LoadDeclsFromFile( void );
	virtual void				WriteDeclFile( void );
	virtual void				FlushDecls( void );
#endif
	...
	void						RegisterDeclFolder( const char *folder, const char *extension, declType_t defaultType, bool unique = false, bool norecurse = false );
	...
	virtual bool					SetPlaybackData( rvDeclPlayback *playback, int now, int control, rvDeclPlaybackData *pbd );
	virtual bool					GetPlaybackData( const rvDeclPlayback *playback, int control, int now, int last, rvDeclPlaybackData *pbd );
	...
	virtual bool					Validate( declType_t type, int index, idStr &report );
...
#if defined(Q4_RECON_ENGINE_PRIVATE) && defined(_M_IX86)
static_assert( sizeof( idDeclType ) == 0x28, "retail idDeclType ABI drift" );
static_assert( sizeof( idDeclFolder ) == 0x44, "retail idDeclFolder ABI drift" );
static_assert( sizeof( idDeclFile ) == 0x38, "retail idDeclFile ABI drift" );
static_assert( sizeof( idDeclLocal ) == 0x60, "retail idDeclLocal ABI drift" );
static_assert( sizeof( idDeclManagerLocal ) == 0x60C, "retail idDeclManagerLocal ABI drift" );
#endif
...
int idDeclFile::LoadAndParse( bool unique ) {
	...
	while( 1 ) {
		...
		if ( token.Icmp( "guide" ) == 0 ) {
			guide = true;
			if ( !src.ReadToken( &token ) ) {
				src.Warning( "Guide without definition at end of file" );
				break;
			}
		}
		...
		if ( unique && stripped.Cmp( name ) != 0 ) {
			common->Warning( "%s must be in a file of the same name!", name.c_str() );
		}

		definition.Clear();
		if ( guide ) {
			if ( !declManagerLocal.EvaluateGuide( name, &src, definition ) ) {
				continue;
			}
		} else if ( !src.ParseBracedSectionExact( definition, -1 ) ) {
			src.Warning( "Type without definition at end of file" );
			break;
		}
		...
		newDecl->SetTextLocal( definition.c_str(), definition.Length() );
...
void idDeclManagerLocal::Init( void ) {
	...
	cmdSystem->AddCommand( "listEffects", idListDecls_f<DECL_EFFECT>, CMD_FL_SYSTEM, "lists effect decls", idCmdSystem::ArgCompletion_String<listDeclStrings> );
...
The retail cache name is derived from the asset-log leaf and uses .decls.
===================
*/
void idDeclManagerLocal::StartLoadingDecls( void ) {
	const char *assetLog = fileSystem->GetAssetLogName();
	const char *leaf = assetLog ? strchr( assetLog, '/' ) : NULL;
	idStr fileName = leaf ? leaf + 1 : ( assetLog ? assetLog : "" );
	fileName.SetFileExtension( ".decls" );
	mSingleDeclFile = fileName.Length() ? fileSystem->OpenFileRead( fileName ) : NULL;
}

void idDeclManagerLocal::FinishLoadingDecls( void ) {
	if ( mSingleDeclFile != NULL ) {
		fileSystem->CloseFile( mSingleDeclFile );
		mSingleDeclFile = NULL;
	}
}

void idDeclManagerLocal::LoadDeclsFromFile( void ) {
	// Transitional: source decl registration remains authoritative until the
	// retail .decls record format and idDeclFile::LoadAndParse(idFile*) land.
	if ( mSingleDeclFile != NULL ) {
		common->DPrintf( "compiled decl cache detected; using source decls during reconstruction\n" );
	}
}

void idDeclManagerLocal::WriteDeclFile( void ) {
	common->Warning( "compiled .decls cache writing is not reconstructed yet" );
}

void idDeclManagerLocal::FlushDecls( void ) {
	for ( int i = loadedFiles.Num() - 1; i >= 0; --i ) {
		for ( idDeclLocal *decl = loadedFiles[i]->decls; decl != NULL; decl = decl->nextInFile ) {
...
void idDeclManagerLocal::RegisterDeclFolder( const char *folder, const char *extension, declType_t defaultType, bool unique, bool norecurse ) {
	...
	// Scan this directory only.  The retail decl manager performs its own
	// recursion below; ListFilesTree already returns paths rooted at `folder`
	// and prepending the folder again produces materials/materials/...
	...
	if ( !norecurse ) {
		...
		for ( i = 0; i < folderList->GetNumFiles(); ++i ) {
			const char *subFolder = folderList->GetFile( i );
			if ( subFolder[0] == '\0' || subFolder[0] == '.' || subFolder[1] == '.' ) {
				continue;
			}
			fileName = folder;
			fileName += "/";
			fileName += subFolder;
			RegisterDeclFolder( fileName, extension, defaultType, unique, false );
		}
...
void idDeclManagerLocal::RegisterDeclFolderWrapper( const char *folder, const char *extension, declType_t defaultType, bool unique, bool norecurse ) {
	...
	const int memory = GetTotalTextMemory( defaultType );
...
const idDecl *idDeclManagerLocal::FindType( declType_t type, const char *name, bool makeDefault, bool noCaching ) {
	...
	decl = FindTypeWithoutParsing( type, name, makeDefault, -1 );
	if ( !decl ) {
		return NULL;
	}
...
rvDeclGuide::rvDeclGuide( idStr &name ) : mName( name ), mNumParms( 0 ) {
}
...
void rvDeclGuide::Parse( idLexer *src ) {
	...
	if ( commas != mNumParms - 1 ) {
		src->Warning( "Guide name '%s' only contains %d commas for %d args, expecting %d! Typo?\n",
			mName.c_str(), commas, mNumParms, mNumParms - 1 );
	}
	src->ParseBracedSectionExact( mDefinition, -1 );
}

void rvDeclGuide::RemoveOuterBracing( void ) {
	const int open = mDefinition.Find( '{' );
	const int close = mDefinition.Last( '}' );
	if ( open >= 0 && close > open ) {
		mDefinition = mDefinition.Mid( open + 1, close - open - 1 );
	}
}

bool rvDeclGuide::Evaluate( idLexer *src, idStr &definition ) {
	idToken token;
	idStr values[MAX_GUIDE_PARMS];
	int numValues = 0;

	if ( !src->ExpectTokenString( "(" ) ) {
		return false;
	}
	while ( numValues < MAX_GUIDE_PARMS && src->ReadToken( &token ) ) {
		if ( token == ")" ) {
			break;
		}
		if ( token == "," ) {
	...
	if ( numValues != mNumParms ) {
		return false;
	}

	definition = mDefinition;
	for ( int i = 0; i < mNumParms; ++i ) {
		definition.Replace( mParms[i], values[i] );
	}
	return true;
}
...
rvDeclGuide *idDeclManagerLocal::GetNewGuide( idLexer *src, idStr &file ) {
	idToken name;
	if ( !src->ReadToken( &name ) ) {
		return NULL;
	}
	rvDeclGuide **existing = NULL;
	if ( guideTable.Get( name.c_str(), &existing ) ) {
		common->Printf( "Guide file '%s' contains duplicate guide '%s'\n", file.c_str(), name.c_str() );
		src->SkipBracedSection( true );
		return NULL;
	}
	idStr guideName = name;
...
void idDeclManagerLocal::ParseGuides( void ) {
	...
	idFileList *guideFiles = fileSystem->ListFiles( "guides", ".guide", true );
	idLexer src;
	src.SetFlags( DECL_LEXER_FLAGS );
	for ( int i = 0; i < guideFiles->GetNumFiles(); ++i ) {
		idStr file = "guides/";
		file += guideFiles->GetFile( i );
		if ( !src.LoadFile( file ) ) {
			fileSystem->FreeFileList( guideFiles );
			common->FatalError( "Couldn't load %s", file.c_str() );
			return;
		}

		idToken token;
		while ( src.ReadToken( &token ) ) {
			const bool inlineGuide = token.Icmp( "inlineGuide" ) == 0;
			if ( !inlineGuide && token.Icmp( "guide" ) != 0 ) {
				common->Printf( "Guide file '%s' has unknown token '%s'\n", file.c_str(), token.c_str() );
				break;
			}
			rvDeclGuide *guide = GetNewGuide( &src, file );
			if ( guide == NULL ) {
				continue;
			}
			if ( inlineGuide ) {
			...
			rvDeclGuide *value = guide;
			guideTable.Set( guide->GetName(), value );
		}
		src.FreeSource();
	}
	fileSystem->FreeFileList( guideFiles );
...
bool idDeclManagerLocal::EvaluateGuide( idStr &name, idLexer *src, idStr &definition ) {
	...
	rvDeclGuide **guide = NULL;
	if ( !guideTable.Get( guideName.c_str(), &guide ) || guide == NULL || *guide == NULL ) {
		src->Warning( "Guide name '%s' not found in '%s'", guideName.c_str(), name.c_str() );
		src->SkipRestOfLine();
		return false;
	}
	return ( *guide )->Evaluate( src, definition );
}

bool idDeclManagerLocal::EvaluateInlineGuide( idStr &name, idStr &definition ) {
	int scanOffset = 0;
	for ( ;; ) {
		idStr source = definition;
		idLexer lexer;
		lexer.LoadMemory( source.c_str(), source.Length(), name.c_str(), 1 );
		lexer.SetFlags( DECL_LEXER_FLAGS );

		idToken token;
		bool expandedOne = false;
		while ( lexer.ReadToken( &token ) ) {
			const int tokenEnd = lexer.GetFileOffset();
			const int tokenStart = tokenEnd - token.Length();
			if ( tokenStart < scanOffset || token.Icmp( "inlineGuide" ) != 0 ) {
				continue;
			}
			idToken guideName;
			...
			idStr expanded;
			if ( !( *guide )->Evaluate( &lexer, expanded ) ) {
				return false;
			}
			const int consumedEnd = lexer.GetFileOffset();
			definition = source.Left( tokenStart );
			definition += expanded;
			definition += source.Right( source.Length() - consumedEnd );
			scanOffset = tokenStart + expanded.Length();
			expandedOne = true;
			break;
		}
		if ( !expandedOne ) {
...
bool idDeclManagerLocal::SetPlaybackData( rvDeclPlayback *playback, int now, int control, rvDeclPlaybackData *pbd ) {
	return playback->SetCurrentData( now * 0.001f, control, pbd );
}
...
bool idDeclManagerLocal::GetPlaybackData( const rvDeclPlayback *playback, int control, int now, int last, rvDeclPlaybackData *pbd ) {
	...
	return playback->GetCurrentData( control, now * 0.001f, last * 0.001f, pbd );
}
...
idStr idDeclManagerLocal::GetNewName( declType_t type, const char *base ) {
	...
	for ( int suffix = 1; suffix < 1024; ++suffix ) {
		...
		if ( FindType( type, name, false, false ) == NULL ) {
			return name;
		}
	}
	return "*unknown*";
...
const char *idDeclManagerLocal::GetDeclTypeName( declType_t type ) {
	return declTypes[type]->typeName.c_str();
}
...
size_t idDeclManagerLocal::ListDeclSummary( const idCmdArgs &args ) {
	...
	for ( int i = 0; i < declTypes.Num(); ++i ) {
		...
		for ( int j = 0; j < num; ++j ) {
			idDeclLocal *decl = linearLists[i][j];
			totalStructs += decl->Size();
			if ( decl->self != NULL ) {
				totalStructs += decl->self->Size();
			}
		}
	}
	common->Printf( "Decls           - %dK in %d decl structs\n", totalStructs >> 10, totalDecls );
...
bool idDeclManagerLocal::Validate( declType_t type, int index, idStr &report ) {
	const idDecl *decl = DeclByIndex( type, index, false );
	if ( decl == NULL || decl->base == NULL ) {
		report = "invalid declaration";
		return false;
	}
	const int length = decl->base->GetTextLength();
	char *text = (char *)_alloca( length + 1 );
	decl->base->GetText( text );
	return decl->Validate( text, length, report );
}

idDecl *idDeclManagerLocal::AllocateDecl( declType_t type ) {
	idDeclLocal *base = new idDeclLocal;
	idDecl *decl = declTypes[type]->allocator();
	decl->base = base;
	base->self = decl;
	base->type = type;
	return decl;
...
=================
*/
bool idDeclLocal::Validate( const char *text, int textLength, idStr &report ) const {
	(void)text;
	(void)textLength;
	report = "_";
	return true;
