...
bool			com_debugHudActive;		//  true while the in-game debug HUD is active
...
#if defined( __DOOM_DLL__ ) || defined( Q4_RECON_ENGINE_PRIVATE )
idGame *		game = NULL;
...
public:
	...
	virtual void				SetCrashReportAutoSendString( const char *string );
...
void idCommonLocal::SetDesiredMachineSpec( int machineSpec ) {
	desiredMachineSpec = machineSpec;
}
...
static void PrintMemInfo_f( const idCmdArgs &args ) {
	MemInfo mi;
	session->GetCurrentMapName( mi.filebase );
...
const char *idCommonLocal::GetLocalizedString( const char *token, int langIndex ) {
	return languageDict.GetString( token );
}

const idLangKeyValue *idCommonLocal::GetLocalizedString( int index, int langIndex ) {
	if ( index < 0 || index >= languageDict.GetNumKeyVals() ) {
		return NULL;
	}
	return languageDict.GetKeyVal( index );
}
...
int idCommonLocal::GetNumLocalizedStrings( void ) const {
	return languageDict.GetNumKeyVals();
}

const char *idCommonLocal::GetLanguage( int index ) const {
	return index == 0 ? sys_lang.GetString() : "";
}
...
int LocalizeMap(const char* mapName, idLangDict &langDict, ListHash& listHash, idStrList& excludeList, bool writeFile) {
	...
	if ( map.Parse(mapName, false, false ) ) {
		...
		if(writeFile && strCount > 0)  {
			...
			bak.Append(".bak_loc");
			::CopyFileA( file.c_str(), bak.c_str(), FALSE );
...
void idCommonLocal::InitRenderSystem( void ) {
	...
	PrintLoadingMessage( common->GetLocalizedString( "#str_104343" ) );
}
...
void idCommonLocal::PrintLoadingMessage( const char *msg ) {
	...
	int len = strlen( msg );
	renderSystem->DrawSmallStringExt( ( 640 - len * SMALLCHAR_WIDTH ) / 2, 410, msg, idStr::ColorForIndex( ':' ), true, declManager->FindMaterial( "gfx/2d/bigchars" ) );
...
void idCommonLocal::LoadGameDLL( void ) {
	...
	if ( game != NULL ) {
		...
		game->Init( Memory::Allocate, Memory::Free, Memory::MSize );
		#else
		game->Init();
		#endif
	}
...
void idCommonLocal::Init( int argc, const char **argv, const char *cmdline ) {
	try {
		...
		if ( !AddStartupCommands() ) {
			...
			session->StartMenu( "playIntro" );
		}

		// Show the game window and hide the splash screen.
		sys->ShowWindow( win32.hWnd, true );
...
void idCommonLocal::InitGame( void ) {
	...
	PrintLoadingMessage( common->GetLocalizedString( "#str_104346" ) );

	// start the sound system, but don't do any hardware operations yet
	soundSystem->Init();

	PrintLoadingMessage( common->GetLocalizedString( "#str_104347" ) );
	...
	if ( idAsyncNetwork::serverDedicated.GetInteger() == 1 ) {
		...
		PrintLoadingMessage( common->GetLocalizedString( "#str_104348" ) );
		InitRenderSystem();
	}
#endif

	PrintLoadingMessage( common->GetLocalizedString( "#str_104349" ) );
	...
	PrintLoadingMessage( common->GetLocalizedString( "#str_104350" ) );

	// load the game dll
	LoadGameDLL();
	
	PrintLoadingMessage( common->GetLocalizedString( "#str_104351" ) );
...
void idCommonLocal::ShutdownGame( bool reloading ) {
	...
	// kill sound first
	soundSystem->StopAllSounds( SOUNDWORLD_ANY );
