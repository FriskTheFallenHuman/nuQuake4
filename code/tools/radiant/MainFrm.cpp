...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext *pContext) {
	...
	if ( !g_Inspectors->Create(IDD_DIALOG_INSPECTORS, this) || !::IsWindow(g_Inspectors->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the inspector window");
		return FALSE;
	}
	...
	const BOOL cameraCreated = m_pCamWnd->Create(CAMERA_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1234);
	if ( !cameraCreated || !::IsWindow(m_pCamWnd->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the camera window (Create=%d, hwnd=%p, error=%lu)",
			cameraCreated, m_pCamWnd->GetSafeHwnd(), GetLastError());
		return FALSE;
	}

	m_pZWnd = new CZWnd();
	if ( !m_pZWnd->Create(Z_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1238) || !::IsWindow(m_pZWnd->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the Z window");
		return FALSE;
	}

	// XYWnd.cpp is retained byte-for-byte from the original Windows source and
	// its legacy class registration leaves hInstance unset.  Modern Win32
	// rejects that WNDCLASS, so register the shared XY/XZ/YZ class correctly
	// before the three MFC wrappers create their windows.
	WNDCLASS xyClass;
	HINSTANCE radiantInstance = AfxGetInstanceHandle();
	if ( !::GetClassInfo(radiantInstance, XY_WINDOW_CLASS, &xyClass) ) {
		memset(&xyClass, 0, sizeof(xyClass));
		xyClass.style = CS_NOCLOSE;
		xyClass.hInstance = radiantInstance;
		xyClass.lpszClassName = XY_WINDOW_CLASS;
		xyClass.lpfnWndProc = ::DefWindowProc;
		if ( !AfxRegisterClass(&xyClass) ) {
			common->Warning("Radiant: failed to register %s (error %lu)", XY_WINDOW_CLASS, GetLastError());
			return FALSE;
		}
	}

	m_pXYWnd = new CXYWnd();
	if ( !m_pXYWnd->Create(XY_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1235) || !::IsWindow(m_pXYWnd->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the XY window");
		return FALSE;
	}
	m_pXYWnd->SetViewType(XY);

	m_pXZWnd = new CXYWnd();
	if ( !m_pXZWnd->Create(XY_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1236) || !::IsWindow(m_pXZWnd->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the XZ window");
		return FALSE;
	}
	m_pXZWnd->SetViewType(XZ);

	m_pYZWnd = new CXYWnd();
	if ( !m_pYZWnd->Create(XY_WINDOW_CLASS, "", QE3_CHILDSTYLE, rect, this, 1237) || !::IsWindow(m_pYZWnd->GetSafeHwnd()) ) {
		common->Warning("Radiant: failed to create the YZ window");
		return FALSE;
	}
...
void CMainFrame::OnActivate(UINT nState, CWnd *pWndOther, BOOL bMinimized) {
	...
	if ( nState != WA_INACTIVE ) {
		...
		soundSystem->SetRenderWorld( g_qeglobals.rw );
	}
	else {
